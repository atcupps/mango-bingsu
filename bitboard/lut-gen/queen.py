...
# moves for a queen on a given rank and file, assuming no pieces block its path.
    ...
    for r in range(8):
        for f in range(8):
            if r == rank and f == file:
                continue
            # Rook-like moves (same rank or same file)
            if r == rank or f == file:
                board[r][f] = 1
            # Bishop-like moves (same diagonals)
            elif abs(r - rank) == abs(f - file):
                board[r][f] = 1
